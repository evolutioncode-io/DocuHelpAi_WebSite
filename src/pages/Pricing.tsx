...
function Pricing() {
  ...
  const checkIcon = (
    <svg className="w-5 h-5 text-[color:var(--color-semantic-success)] mx-auto" fill="currentColor" viewBox="0 0 20 20">
      <path
        fillRule="evenodd"
  ...
  const crossIcon = (
    <svg className="w-5 h-5 text-[color:var(--color-neutral-400)] mx-auto" fill="currentColor" viewBox="0 0 20 20">
      <path
        fillRule="evenodd"
    ...
    <div className="min-h-screen bg-app">
      <section className="relative bg-surface py-20 overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-6 text-center relative">
          ...
          <div className="flex justify-center gap-4 text-sm text-body flex-wrap">
            {[t('hero.features.noCommitment'), t('hero.features.cancelAnytime'), t('hero.features.spanishSupport')].map((text) => (
              <div key={text} className="flex items-center gap-2">
                <svg className="w-5 h-5 text-[color:var(--color-semantic-success)]" fill="currentColor" viewBox="0 0 20 20">
                  <path
                    fillRule="evenodd"
                    d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
                    clipRule="evenodd"
      ...
      <section className="max-w-7xl mx-auto px-6 py-20">
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="card-surface card-hover-blue border-2 border-[color:var(--color-neutral-400)] p-8 hover:shadow-2xl transition-all cursor-pointer">
            <div className="mb-6">
              <h2 className="text-2xl font-bold mb-2 text-heading">{t('plans.starter.name')}</h2>
              <p className="text-body">{t('plans.starter.description')}</p>
            </div>
            <div className="mb-8">
              <div className="flex items-baseline gap-2 mb-2">
                <span className="text-5xl font-bold text-heading">{t('plans.starter.price')}</span>
              </div>
              <p className="text-body text-sm">{t('plans.starter.priceSubtext')}</p>
            </div>
            <div className="space-y-4 mb-8">
              {[
                ...
                <div key={item} className="flex items-start gap-3">
                  <svg className="w-6 h-6 text-[color:var(--color-semantic-success)] flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                    <path
                      fillRule="evenodd"
                      d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
                      clipRule="evenodd"
            ...
            <Link to={getLocalizedRoute('contact')}>
              <button className="w-full bg-[color:var(--color-neutral-900)] text-white py-4 rounded-xl font-semibold hover:opacity-90 transition-opacity shadow-lg" aria-label={t('plans.starter.button')}>
                {t('plans.starter.button')}
              </button>
            </Link>
          </div>

          <div className="bg-gradient-secondary rounded-3xl shadow-2xl p-8 transform lg:-translate-y-4 relative text-white">
            <div className="absolute -top-4 left-1/2 -translate-x-1/2">
              <span className="bg-[color:var(--color-semantic-warning)] text-[color:var(--color-neutral-900)] px-4 py-1 rounded-full text-sm font-bold">{t('plans.professional.badge')}</span>
            </div>
            <div className="mb-6">
            ...
            <Link to={getLocalizedRoute('contact')}>
              <button className="w-full bg-white text-[color:var(--color-accent-700)] py-4 rounded-xl font-bold hover:bg-gray-50 transition-colors shadow-lg" aria-label={t('plans.professional.button')}>
          ...
          <div className="card-surface card-hover-blue border-2 border-[color:var(--color-neutral-400)] p-8 hover:shadow-2xl transition-all cursor-pointer">
            <div className="mb-6">
              <h2 className="text-2xl font-bold mb-2 text-heading">{t('plans.enterprise.name')}</h2>
              <p className="text-body">{t('plans.enterprise.description')}</p>
            </div>
            <div className="mb-8">
              <div className="flex items-baseline gap-2 mb-2">
                <span className="text-5xl font-bold text-heading">{t('plans.enterprise.price')}</span>
              </div>
              <p className="text-body text-sm">{t('plans.enterprise.priceSubtext')}</p>
            </div>
            <div className="space-y-4 mb-8">
              {[
            ...
            <Link to={getLocalizedRoute('contact')}>
              <button className="btn-secondary w-full py-4" aria-label={t('plans.enterprise.button')}>
      ...
      <section className="max-w-6xl mx-auto px-6 py-20">
        ...
        <div className="card-surface rounded-2xl shadow-xl overflow-hidden">
          <table className="w-full">
            <thead className="bg-gradient-secondary text-white">
              <tr>
            ...
            <tbody className="divide-y divide-[color:var(--color-neutral-400)]">
              {[
                ...
                <tr key={row.label} className="hover:bg-surface-muted">
                  <td className="px-6 py-4 font-medium text-heading">{row.label}</td>
                  {['starter', 'professional', 'enterprise'].map((tier) => (
                    <td key={tier} className="px-6 py-4 text-center text-body">
      ...
      <section className="max-w-4xl mx-auto px-6 py-20">
        <h2 className="text-3xl font-bold mb-12 text-center text-heading">{t('faq.title')}</h2>
        <div className="space-y-6">
          {[
            ...
            <article key={faq.question} className="card-surface card-hover-blue border-2 border-[color:var(--color-neutral-400)] p-6 rounded-xl cursor-pointer" itemScope itemType="https://schema.org/Question">
              <h3 className="font-bold text-lg mb-2 text-heading" itemProp="name">{faq.question}</h3>
              <div itemScope itemType="https://schema.org/Answer" itemProp="acceptedAnswer">
                <p className="text-body" itemProp="text">{faq.answer}</p>
              </div>
            </article>
          ))}
        </div>
      ...
      <section className="bg-gradient-secondary py-20">
        <div className="max-w-4xl mx-auto text-center px-6">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <Link to={getLocalizedRoute('contact')}>
              <button className="bg-white text-[color:var(--color-accent-700)] px-10 py-5 rounded-xl shadow-2xl hover:shadow-3xl transform hover:-translate-y-1 transition-all duration-200 font-bold text-lg" aria-label={t('cta.buttonPrimary')}>
                {t('cta.buttonPrimary')}
              </button>
            </Link>
            <Link to={getLocalizedRoute('contact')}>
              <button className="bg-[color:var(--color-neutral-900)] text-white px-10 py-5 rounded-xl shadow-2xl hover:opacity-90 transition-all duration-200 font-bold text-lg" aria-label={t('cta.buttonSecondary')}>
